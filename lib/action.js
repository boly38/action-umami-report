...
class Action {
  ...
  static async produceReport(umamiSite, umamiSiteStats, sitePageViews = null, siteEvents = null, siteMetricsUrl = null,
      outputFile = null, reportContent = 'pageviews|events|urls') {
    //~~ generate umami report content
    const generator = new ReportGenerator(umamiSite, reportContent, umamiSiteStats, sitePageViews, siteEvents, siteMetricsUrl);
  ...
  static async umamiDailyReportV0(server, user, password, domain = '*first*', outputFile = null, reportContent = 'pageviews|events|urls') {
    ...
    const siteMetricsUrl = await umamiClient.getMetricsForLast24h(authData, site, { type: 'url' } ).catch(rethrow);
