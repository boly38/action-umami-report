...
import UmamiAPI from 'umami-api'
...
class Action {
  ...
  static async umamiReport(options) {
    ...
    const umami = await new UmamiAPI(server).auth(user, password).catch(rethrow);
    const site = isSet(domain) ? await umami.getWebsite("domain", domain).catch(rethrow) : await umami.getWebsite().catch(rethrow);
    ...
    const siteMetricsUrl = await umami.getMetrics(site.website_id, period, { type: 'url' }).catch(rethrow);
