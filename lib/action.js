...
class Action {
  ...
  static async produceReport(umamiSite, umamiSiteStats, sitePageViews = null, siteEvents = null, outputFile = null) {
    //~~ generate umami report content
    const generator = new ReportGenerator(umamiSite, umamiSiteStats, sitePageViews, siteEvents);
  ...
  static async umamiDailyReportV0(server, user, password, domain = '*first*', outputFile = null) {
    ...
    const siteEvents = await umamiClient.getEventsForLast24h(authData, site).catch(rethrow);

    DEBUG_ACTION && console.log(site);
    DEBUG_ACTION && console.log(siteStats);
    const targetFile = await Action.produceReport(site, siteStats, sitePageViews, siteEvents, outputFile);
    if (targetFile != null) {
